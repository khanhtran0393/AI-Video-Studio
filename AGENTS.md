...
| `npm run test:viral-cut` | Viral Cut engine unit test (24 test: JSON lỏng lẻo, heuristic, energy, hook, export plan, concat plan) |
...
| `npm run test:ffx-smoke` | smoke Công cụ FFmpeg (`nova/scripts/ffx-smoke.js`) — chạy ffmpeg/ffprobe THẬT trên video app đã tạo trong `output/gen-e2e/` (tách audio, cắt, ghép copy/auto/xfade, loop, nén, trích frame, GIF, addMusic, Shorts 9:16, đóng phụ đề SRT/ASS + tự chuyển UTF-16→UTF-8, xem thử phụ đề 1 khung, faststart (+ already skip), loudnorm 2-pass (+ keepVideo), bỏ lời/tách giọng (+ loudnorm ghép), fade (+ video copy khi chỉ fade tiếng), huỷ, validate lỗi lộ liễu); thiếu dữ liệu nguồn → FAIL, không bịa dữ liệu |
