 ...
 *      artifacts/san-pham-that-2026-09-02T04-33-44-395Z/04-video-agent/du-an-ha-long/tts/chapter-001.mp3
 ...
 *
 * Chạy:  Set-Location 'D:\AI Video Studio\nova'; node documentary/test-word-sync-e2e.js
...
const SOURCE_DIR = path.join(WORKSPACE, 'artifacts', 'san-pham-that-2026-09-02T04-33-44-395Z', '04-video-agent', 'du-an-ha-long');

const REAL_TTS = path.join(SOURCE_DIR, 'tts', 'chapter-001.mp3');
const REAL_IMAGES = [
  path.join(SOURCE_DIR, 'images', 'canh-1.png'),
  path.join(SOURCE_DIR, 'images', 'canh-2.png'),
  path.join(SOURCE_DIR, 'images', 'canh-3.png'),
];
const WHISPER_JSON = path.join(RUN_DIR, 'whisper-words.json');
const OUTPUT_MP4 = path.join(RUN_DIR, 'out-word-sync.mp4');
const REPORT_JSON = path.join(RUN_DIR, 'e2e-report.json');
...
function realAssets() {
  ...
  return [
    { id: 'img1', type: 'image', path: REAL_IMAGES[0], title: 'Giọng gốc tiếng Việt', tags: ['giong', 'goc', 'tieng', 'viet', 'tiengviet'] },
    { id: 'img2', type: 'image', path: REAL_IMAGES[1], title: 'Máy tổng hợp hoàn toàn', tags: ['may', 'tong', 'hop', 'hoan', 'toan', 'tonghop'] },
    { id: 'img3', type: 'image', path: REAL_IMAGES[2], title: 'AI Video Studio New', tags: ['ai', 'video', 'studio', 'new', 'may'] },
  ];
}
...
async function main() {
  ...
  const project = await orchestrator.run({
    rootDir: RUN_DIR, projectId: 'word_sync_e2e',
    input: {
  ...
  for (const beat of syncedBeats) {
    ...
    for (const ev of events) {
      const src = alignment.words.find(w => w.word === ev.word);
  ...
  const projectB = await orchestrator.run({
    rootDir: RUN_DIR, projectId: 'word_sync_e2e_det',
  ...
  console.log('\nWORD-SYNC-E2E-OK — đồng bộ theo TỪ đã hoạt động trên sản phẩm thật (audio TTS + ảnh + render mp4).');
}

main().catch(error => { console.error('WORD-SYNC-E2E-FAIL', error && error.stack || error); process.exitCode = 1; });
