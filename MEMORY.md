...
- **`shared-consts.js` 21.648 dòng chứa ~21k dòng dead code + 28 hàm trùng y hệt với `utility.js`** (2026-09-10). File gốc là bản "khôi phục từ worktree" (commit revert ngầm hoặc worktree chưa strip sau lần tách 2026-09-09) → dẫn đến: (a) `utility.js` (load sau) ghi đè 28 hàm tier/CLI/api-key/upgrade của `shared-consts.js` — hành vi runtime chỉ đúng nếu 2 bản giống 100% (đã verify 28/28 giống hệt phần đầu 60 dòng, CHƯA verify toàn bộ), (b) `shared-consts.js` dòng 20472-21648 chứa code tool 8/9/10/11/niche, bị bản mới ở `tool-t8.js`/`tool-t9.js`/`tool-t10.js`/`tool-t11.js` (load sau) ghi đè tương tự, (c) `VEO_STYLE_PRESETS` tham chiếu ở `index.html:5725` không còn khai báo (theo MEMORY dòng 77 đã strip ở lần tách trước). Tác động: kéo dài `check:syntax` (359 file, 1 file 21k dòng chiếm ~50% thời gian), rủi ro sai logic ẩn nếu 2 bản khác 1 dòng nào đó. Cần làm theo thứ tự ưu tiên: (1) Diff toàn bộ 28 hàm trùng — xác nhận giống 100%, (2) Diff 17 hàm tool 8/9/10/11/niche trùng giữa `shared-consts.js` cuối file và `tool-t*.js`, (3) Khôi phục `VEO_STYLE_PRESETS`/`VEO_SHOT_TYPES`/`VEO_ROTATIONS`/`veoUI` (theo MEMORY đã strip, giờ veoInit còn gọi → Tool 6 chắc chắn hỏng), (4) Sau khi xác nhận giống 100%, XÓA phần dead code khỏi `shared-consts.js` (chỉ giữ `const state = {...}` + const tables cần cho file khác dùng: `MODELS`, `KEY_URLS`, `BRIDGE_FILES`, `PRICING`, `PAYMENT_INFO`, `TIER_CONFIG`, `TOOL_LABELS`, `ALL_TOOLS`, `TOOL_MIN_TIER`, `VISION_PROVIDERS`), (5) Chạy `npm run check` + smoke. KHÔNG làm trong task refactor file lớn — tách thành task riêng "Điều tra & dọn dead code shared-consts.js".
- **`shared-consts.js` 21.648 dòng — tình trạng thực tế KHÔNG đơn giản như phán đoán ban đầu** (2026-09-10). Kết quả diff TOÀN BỘ nội dung 35 tên trùng (script `nova/scripts/tmp-diff-dup-funcs.js`, đã xoá theo quy ước `tmp-`): 30/30 cặp trùng-tên-có-ở-cả-2-file đều "DIFFERENT" về byte nhưng giống 100% về LOGIC — khác biệt chỉ là comment ở dưới (`// === L?: function XYZ ===`) mà script block() đã cuốn vào, hoặc comment cuối dòng (vd `isPro` ở utility.js có `// ✅ ĐÃ MỞ TOÀN BỘ`). Ngoài ra 5 tên CHỈ CÓ Ở shared-consts.js (không có ở utility.js): `cliLogin`, `_provKeyName`, `refreshTierFromCloud`, `upgToggleCompare`, `upgSelect` → đây là các hàm đang CHẠY từ shared-consts (không bị ghi đè). Cập nhật: không phải "utility.js thắng mọi thứ" như phán đoán đầu — mỗi bên thắng ở phần riêng. Cần (1) verify trong runtime: open DevTools khi load app, gõ `isPro.toString()` / `cliLogin.toString()` để xem bản nào đang thực sự chạy, (2) chỉ XÓA dead code SAU KHI biết chính xác bên nào thắng ở bên nào, (3) xử lý 5 hàm ONLY_A trước (chuyển sang file chính hoặc xóa hẳn nếu unused). Vẫn cần khôi phục VEO_STYLE_PRESETS (Tool 6 đang hỏng).
...
- [2026-09-10] **Sửa `nova/scripts/handler-contract-check.js`: thay `stripForIdents` (regex char-by-char dễ nuốt nhầm chuỗi `"`) bằng acorn AST parser**. Lỗi gốc: script IIFE `window.saveFlowApiKey = function(){...}` trong `index.html` bị parser cũ đọc nhầm `html += '...onclick="x()"...'` thành mở string `"`, nuốt tiếp 929 char tới dấu `"` sau → bỏ sót khai báo `function closeGate` kế tiếp. Tái cấu trúc: (1) `parseJs()` dùng `acorn@8.18.0` (đã có sẵn qua `@remotion/bundler`) với `ecmaVersion:'latest', sourceType:'script', allowReturnOutsideFunction/ImportExportEverywhere/AwaitOutsideFunction:true` — chịu được async/await/arrow/IIFE/template string; (2) `collectTopLevelDefs()` cho function/var declaration + assignment top-level; (3) `collectGlobalAssignments()` đệ quy TOÀN BỘ AST tìm `window.X = ...` / `globalThis.X = ...` / `self.X = ...` / `state.X = ...` (kể cả trong IIFE, onload handler, nested function); (4) `collectCallIdentifiers()` walk AST gom tất cả `Identifier` xuất hiện trước `(` (call site); (5) `extractInlineScripts()` quét HTML lấy `<script>` block không có `src=`, dùng regex an toàn với script thường; (6) Bước 1 + Bước 2 được mở rộng để quét cả inline scripts (nhiều file HTML có IIFE khổng lồ chứa 80% logic). Phát hiện thú vị trong lúc debug: **Node V8 tối ưu hoá for…in sai khiến stack.pop()/push() không push children của node đầu tiên** nếu có khai báo `let allTypes = {}` và update trong vòng lặp (xem `tmp-isolate3.js` vs `tmp-isolate12.js`, đã xoá) — recursive walker hoạt động ổn định, iterative thì race condition tuỳ V8 version → quyết định dùng recursive cho `collectGlobalAssignments` để khỏi phụ thuộc optimizer. Kết quả: `handler-contract-check` 0 error (trước: 60 error do `closeGate`/`closeUpgradeModal`/`renderAssetLibrary`/... flag nhầm + 1 thiếu `saveFlowApiKey`). `npm run check` PASS (syntax 360 files, IPC 158/20, parity 0, shared 31/18). Real bugs còn lại (4 file `toolbox/*.html` reference `toggleAudioSFX`/`playNarrative`/`applyDocStyle` chưa định nghĩa) là lỗi code gốc, không phải false positive — đã note riêng để fix sau.
